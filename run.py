...
async def detect_and_store_raw_outliers(
    session: AsyncSession, target_email: str, iteration_num: int
) -> dict:
    logger.info(
        f"Iteration {iteration_num}: starting outlier detection for {target_email}"
    )
    ...
    for data_type, rec_vals in grouped.items():
        await redis_client.set(
            f"{settings.REDIS_FIND_OUTLIERS_JOB_IS_ACTIVE_NAMESPACE}{target_email}",
            "true",
        )
    ...
    logger.info(
        f"Iteration {iteration_num}: committed {len(outliers_to_add)} outliers to DB"
    )
...
async def detect_and_store_processed_outliers(
    ...
    """
    logger.info(
        f"Iteration {iteration_num}: starting processed-records outlier detection for {target_email}"
    )
    ...
    for rec in records:
        try:
            ...
            logger.warning(
                f"Skipping non-numeric value in processed record id={rec.id}"
            )
            continue
    ...
    for data_type, rec_vals in grouped.items():
        ...
        logger.info(
            f"[Processed] DataType '{data_type}': found {cnt} outliers out of {len(vals)}"
        )
    ...
    await session.commit()
    logger.info(
        f"Iteration {iteration_num}: committed {len(to_add)} processed-records outliers"
    )
...
async def main(target_email: str):
    ...
    async with get_session() as session:
        ...
        try:
            await send_outlier_start_notification(
                target_email, iteration_number, start_time
            )
        except Exception:
            logger.error("failed to send notification")

        raw_summary = await detect_and_store_raw_outliers(
